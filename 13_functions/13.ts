	...
	parameters: {
		...
		properties: {
			command: {
				type: 'boolean',
				description:
					"Set to 'true' when query is direct command for AI. Set to 'false' when queries asks for saying/writing/translating/explaining something and all other.",
			},
			type: {
				type: 'string',
				description:
					"memory (queries about the user and/or AI), notes|links (queries about user's notes|links). By default pick 'memory'.",
				enum: ['memory', 'notes', 'links'],
			},
			tags: {
				type: 'array',
				description:
					"Multiple semantic tags/keywords that enriches query for search purposes (similar words, meanings). When query refers to the user, add 'overment' tag, and when refers to 'you' add tag 'Alice'",
				items: {
					type: 'string',
				},
			},
...
const result = await model.invoke([new HumanMessage('Hey there!')]);
