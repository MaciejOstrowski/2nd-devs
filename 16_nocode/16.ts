...
const todoist = async (manager: { args: any }) => {
	return await fetch(`https://hook.eu1.make.com/oltalo644wj7l2b484j4zyfan638omf2`, {
		method: 'POST',
...
const act = async (command: string) => {
	...
	const add = await model.invoke([new SystemMessage('Fact: Today is 09/22/2023 20:01.'), new HumanMessage(command)]);
